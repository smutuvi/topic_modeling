...
def read_text( in_path ):
	...
	body = ""
	while True: ##comment this when for loop is in use
	# for line in f:
		##line = line.replace( "[ GS_aligned]", "[GS_aligned]" ) #use to replace a term in the corpus
		# if line.startswith("[OCR_aligned]"):
		# 	line = line[len( "[OCR_aligned]" ) + 1:]
			line = f.readline() ##comment this when for loop is in use
			if not line:
				break
